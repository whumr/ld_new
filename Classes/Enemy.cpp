...
void Enemy::update(float time){
    ...
    timeCount1++;
    CallFunc *func = CallFunc::create(CC_CALLBACK_0(Enemy::enemyShoot, this));
    auto size = Director::getInstance()->getWinSize();
    switch (way) {
        case 0://下落
        {
    ...
    switch(way)
    {
        ...
        case 2:
        {
            ...
            switch(randomType)
            {
                case 1:
                {
                    this->schedule(schedule_selector(Enemy::shootMany));
                    break;
                }
                case 2:
                {
...
void Enemy::shootMany(float f)
{
    ...
    {
        shootManyCount = 0;
        this->unschedule(schedule_selector(Enemy::shootMany));
    }
}

void Enemy::shootFive(Point playerPoint)
{
...
void Enemy::shootOne(Point playerPoint)
{
...
void Enemy::enemyDead()
{
    ...
    //从敌人数组将被攻击的敌怪删除
    Vector<Enemy*> array = LevelScene::sharedLevelScene()->getArrayForEnemy();
