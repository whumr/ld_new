...
GameLayer * GameLayer::sharedLevelScene(){
    if(sh){
        return sh;
    }
    return NULL;
...
bool GameLayer::init()
{
    ...
    //创建子弹逻辑
    this->schedule(schedule_selector(GameLayer::autoCreateBullet));
    ...
    //添加敌人
    this->schedule(schedule_selector(GameLayer::autoCreateEnemy));
    
    //添加宝石
    this->schedule(schedule_selector(GameLayer::autoCreateGems),15);
    
//    this->setTouchEnabled(true);
    
    MenuItemImage *pause = MenuItemImage::create("pause.png", "pause.png", CC_CALLBACK_1(GameLayer::doPause, this));
    ...
    this->schedule(schedule_selector(GameLayer::resetPlayer));


	auto _listener_touch = EventListenerTouchOneByOne::create();
	_listener_touch->onTouchBegan = CC_CALLBACK_2(GameLayer::TouchBegan, this);
	_listener_touch->onTouchMoved = CC_CALLBACK_2(GameLayer::TouchMoved, this);
...
//暂停游戏
void GameLayer::doPause(Ref* psend)
{
...
bool GameLayer::TouchBegan(Touch *pTouch, Event *pEvent)
{
    return true;
}

void GameLayer::TouchMoved(cocos2d::Touch *pTouch, cocos2d::Event *pEvent)
{
    //我们获取主要飞机作为触摸跟随对象
...
//添加飞机属性
void GameLayer::addPlaneProperty()
{
...
//自动添加主角子弹
void GameLayer::autoCreateBullet(float f)
{
...
//自动产生敌人
void GameLayer::autoCreateEnemy(float f)
{
    allTimeCount++;
    ...
    {
        enemyType = 1;
        this->schedule(schedule_selector(GameLayer::setEnemy));
    }else
    {
...
void GameLayer::setEnemy(float f)
{
    auto size = Director::getInstance()->getWinSize();
    switch(enemyType)
    {
...
void GameLayer::autoCreateGems(float f)
{
	CallFunc *func1 = CallFunc::create(CC_CALLBACK_0(GameLayer::putGems, this));
    CallFunc *func2 = CallFunc::create(CC_CALLBACK_0(GameLayer::removeSupplyPlane, this));
    auto size = Director::getInstance()->getWinSize();
...
void GameLayer::removeSupplyPlane()
{
    this->removeChildByTag(100, true);
}

void GameLayer::resetPlayer(float f)
{
    switch(playerType)
    {
        case 1:
...
void GameLayer::setPlayer(int i)
{
    if(!isPlayerChange)
    {
        return;
    ...
    int killCount = this->getPlayer()->killCount;
    this->unschedule(schedule_selector(GameLayer::autoCreateBullet));
...
void GameLayer::lostGame()
{
    ...
    MenuItemLabel* menuLabel= MenuItemLabel::create(ttback, CC_CALLBACK_1(GameLayer::backMenu, this));
...
//菜单项"返回主菜单"的回调函数
void GameLayer::backMenu(Ref* psend)
{
...
//获取主角
NewPlayer* GameLayer::getPlayer()
{
    NewPlayer* player = (NewPlayer*)GameLayer::sharedLevelScene()->getChildByTag(tag_player);
    return player;
}

//获取敌人数组
Vector<NewEnemy*> GameLayer::getArrayForEnemy()
{
    return arrayEnemy;
}
